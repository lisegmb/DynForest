...
Note that for both `compute_vimp()` and `compute_gvimp()`, the argument `permute_trajectory = TRUE` allows permuting entire longitudinal trajectories between subjects. By default (`FALSE`), the values of each marker are permuted observation by observation. This option is useful when the goal is to preserve the temporal correlation structure within each trajectory while assessing variable importance.
...
To further investigate the effect of predictors on the predicted outcome, the `compute_ice_pdp()` function allows to quantify and visualize the relationship between a predictor and the predicted cumulative incidence function.   

- **Partial Dependence Plot (PDP)** provides the average effect of a predictor on the predicted outcome, marginalizing over all other predictors.  
- **Individual Conditional Expectation (ICE)** plots display the effect of a predictor for each individual, highlighting subject-specific heterogeneity in predictor effects.  

Using an object returned by `compute_ice_pdp()`, the `plot()` function allows to visualize both ICE and PDP curves. The `type` argument defines whether ICE or PDP curves are displayed (or both).

Optionally, the `plot()` function can display confidence bands around the PDP curve. These bands are computed from the empirical variability of the Individual Conditional Expectation (ICE) curves. For each value of the predictor, the PDP corresponds to the mean of the ICE values across individuals, and pointwise confidence intervals are constructed using a Student approximation based on the standard error of this mean. Ongoing work is currently investigating whether these bands provide adequate statistical coverage of the true underlying partial dependence function.

```{r, eval=FALSE, echo=TRUE}
res_cat_age <- compute_ice_pdp(
    model     = res_dyn,
    var_name = "age",
    grid_type = "regular"
  )
plot(res_cat_age, type = "both", conf_band = TRUE)

```

```{r, echo=FALSE, fig.show = 'hold', fig.align='center'}
knitr::include_graphics("Figures/DynForestR_classi_ICE.png")
knitr::include_graphics("Figures/DynForestR_classi_PDP.png")
